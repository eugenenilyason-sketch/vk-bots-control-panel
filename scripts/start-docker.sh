...
if [ ! -f ".env" ]; then
    ...
    if grep -q "your_" .env; then
        ...
        POSTGRES_PASS=$(openssl rand -hex 32 2>/dev/null || echo "dev_postgres_password_change_me")
        JWT_SECRET=$(openssl rand -hex 32 2>/dev/null || echo "dev_jwt_secret_change_me")
        NC_JWT_SECRET=$(openssl rand -hex 32 2>/dev/null || echo "dev_nocodb_jwt_secret_change_me")
        N8N_JWT_SECRET=$(openssl rand -hex 32 2>/dev/null || echo "dev_n8n_jwt_secret_change_me")
        REDIS_PASS=$(openssl rand -hex 32 2>/dev/null || echo "dev_redis_password_change_me")
