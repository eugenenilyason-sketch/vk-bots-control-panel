        ...
        <div class="form-group">
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 12px; margin-top: 8px;">
                @if(isset($methods) && count($methods) > 0)
                    ...
                    <label style="padding: 20px; border: 2px solid var(--border-color); border-radius: 8px; cursor: pointer; text-align: center; transition: all 0.2s; {{ old('method') === $method['id'] ? 'border-color: var(--accent-primary); background: rgba(16, 185, 129, 0.1);' : '' }}">
                        <input type="radio" name="method" value="{{ $method['id'] }}" {{ old('method') === $method['id'] || (!old('method') && $loop->first) ? 'checked' : '' }} style="display: none;">
                        <div style="font-size: 40px; margin-bottom: 8px;">{{ $method['icon'] }}</div>
